...
function indekseGoreFenomen(fenomenDizi, fenomenIndex) {
  const result = `${fenomenIndex}. indekste bulunan fenomen: ${fenomenDizi[fenomenIndex]["profile"]}`;
  return result;}
...
function profilListesi(fenomenDizi) {
  let kopyaDizi = fenomenDizi.slice();
...
function fenomenSil(fenomenDizi, fenomenIndeks) {
  let kopyaDizi = fenomenDizi.slice();
  kopyaDizi.splice(fenomenIndeks,1);
  return kopyaDizi;
}
...
function fenomenEkle(fenomenDizi,number,profile,followers,posts,platform) {
  let kopyaDizi = fenomenDizi.slice();
  let yeniFenomen = { 
  "number": number,
  "profile": profile,
...
function enFenomenler(fenomenDizi) {
  let kopyaDizi = [];
  for (let i = 0; i < fenomenDizi.length; i++) {
    if (fenomenDizi[i]["followers"]>100000000) {
      kopyaDizi.push(fenomenDizi[i]["profile"]);
    }       
  }
  return kopyaDizi;
}
...
function fenomenGonderimSayisi(fenomenDizi, fenomenProfil){
  let postSayi=0;
  for (let i = 0; i < fenomenDizi.length; i++) {
    if (fenomenDizi[i]["profile"] === fenomenProfil) {
      postSayi = fenomenDizi[i]["posts"];
    }       
  }
  return postSayi;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenDizi, platformAdi){
  let max = 0;
  let isim;
  for (let i = 0; i < fenomenDizi.length; i++) {
    if (fenomenDizi[i]["platform"] === platformAdi) {
      if (fenomenDizi[i]["posts"]>max) {
        max = fenomenDizi[i]["posts"]
        isim = fenomenDizi[i]["profile"]
      }
    }    
  }
  return isim;
